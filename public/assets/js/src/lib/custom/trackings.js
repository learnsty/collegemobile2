/** 
 * @file: trackings.js
 * @desc: This tracks most things on the CollegeMobile Web Application {Erros, Activites, }
 *
 * CollegeMobile™ Copyright (c) 2016. 
 * All rights reserved 
 */
...
;(function(w, d, $cdvjs){
    ...
    window.Futures = $cdvjs.Futures || (function(){
                 // Promise Interface
            return function(){
	...
	window.ErrorLogger = (function(){
	             // Error Log Interface (Client-Side Error Tracking) 
				 function Logger(config){
					   ...
					   this.remoteDriver = this.config.driver || new Image(); 
					   this.currentErrorObj = null;
					   this.promiseObj = null;

					   var hasW3CEvent = ('addEventListener' in win);
					   var hasWHATWGEvent = ('onerror' in win);
					   var original = hasWHATWGEvent && win.onerror;
				...
				Logger.prototype.log = function(logType, details, stack){
				  if((!this.config.disabled) && (!!this.defaultDriver)){
				   ...
				   if((!this.config.disabled) && (!!this.removeDriver)){
				   	 this.remoteDriver.onload = this.remoteDriver.onerror = function(){ };
				   	 // "http://appmonitor.collegemobile.net/viewings/errors/?browser=Firefox&_ump=13766381938"
				   	 // "http://appmonitor.collegemobile.net/viewings/analytics/?"
                     this.remoteDriver['src'] = "http://appmonitor.collegemobile.net/trackings/errors/?message="+details.message+"&file="+details.file+"&line="+details.lineNumber;
				   }
...
TraceKit.computeStackTrace = (function computeStackTraceWrapper() {
    ...
    function escapeCodeAsRegExpForMatchingInsideHTML(body) {
        return escapeRegExp(body).replace('<', '(?:<|&lt;)').replace('>', '(?:>|&gt;)').replace('&', '(?:&|&amp;)').replace
...
('"', '(?:"|&quot;)').replace(/\s+/g, '\\s+');
    ...
    function findSourceByFunctionBody(func) {
        ...
        // corpus large enough to confirm that and it was in the original.
            ...
            body = escapeRegExp(parts[3]).replace(/;$/, ';?'); // semicolon is inserted if the function ends with a 

comment.replace(/\s+/g, '\\s+');
    ...
    function computeStackTraceFromStackProp(ex) {
        ...
        var chrome = /^\s*at (.*?) ?\(((?:file|https?|blob|chrome-extension|native|webpack|eval).*?)(?::(\d+))?(?::(\d+))?

\)?\s*$/i,
            gecko = /^\s*(.*?)(?:\((.*?)\))?(?:^|@)((?:file|https?|blob|chrome|webpack|\[native).*?)(?::(\d+))?(?::(\d+))?

\s*$/i,
            winjs = /^\s*at (?:((?:\[object object\])?.+) )?\(?((?:ms-appx|https?|webpack|blob):.*?):(\d+)(?::(\d+))?\)?\s*
...
$/i,
    ...
    function computeStackTraceFromStacktraceProp(ex) {
        ...
        var opera10Regex = / line (\d+).*script (?:in )?(\S+)(?:: in function (\S+))?$/i,
            opera11Regex = / line (\d+), column (\d+)\s*(?:in (?:<anonymous function: ([^>]+)>|([^\)]+))\((.*)\))? in (.*):
...
\s*$/i,
    ...
    function computeStackTraceFromOperaMultiLineMessage(ex) {
        ...
        // Backtrace:
        //   Line 3 of linked script file://localhost/Users/andreyvit/Projects/TraceKit/javascript-client/sample.js: In 
        ...
        for (var line = 2; line < lines.length; line += 2) {
            ...
            if ((parts = lineRE1.exec(lines[line]))) {
                ...
                item = {
                    ...
                    'args': [],
                    'line': +parts[1],
                    'column': null // TODO: Check to see if inline#1 (+parts[2]) points to the script number or column 
...
"\n\nContext:\n" + context.join("\n") + "\n\nURL:\n" + item.url);
    ...
    function computeStackTraceByWalkingCallerChain(ex, depth) {
        ...
        augmentStackTraceWithInitialElement(result, ex.sourceURL || ex.fileName, ex.line || ex.lineNumber, ex.message || 
