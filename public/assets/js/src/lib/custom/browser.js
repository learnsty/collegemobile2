/*!
 * @project: <synergixe - SynTestRepo>
 * @file: <browser.js>
 * @author: <https://team.synergixe.com.ng>
 * @created: <01/07/2015>
 * @desc: {this is a browser detection script}
...
contentLoaded.apply(null, [window, function(){ 
	 ...
	 w=this, d=w.document, rt = d.documentElement,  dd="documentMode", n= w.navigator, ua = n.userAgent; // global objects...
	
	var OS = {                     //detecting OS data...
	  isLinux:function(){ 
	...
	isOperaMobile:function(bd){
	    ...
	    return (ua.indexOf("Opera Mini") > 0) && ('OMiniFold' in bd.style);
		
	},
	isIOS:function(bd){ 
	
	    return (ua.indexOf("iPhone; U;") > 0) || (ua.indexOf("iPad; U;") > 0) || (ua.indexOf("iPod; U;") > 0);
    ...
    onMobile:function(){
	    ...
	    return (ua.match(/[^-]mobi|mobile/i));
	
	}	
	}; 
	
	var browserName, borwserVersion, isChrWebkit=false, isSafWebkit=false, isOperaWebkit=false, isKDE = false, nk = ua.toLowerCase(),
    ...
    var isPresto = (/Presto/g.test(ua) && typeof(d.evaluate) == "function" && (('OLink' in body.style) || to_string(w.opera) == "[object opera]") && w.history.navigationMode);
    ...
    dpi = w.screen.colorDepth; // always '32'
    ...
    switch(screenfactor){
		  ...
		  case "0.7500": // screendim - 768x1024, screendim - 600x800, screendim - 360x480 - Portrait
		        if((OS.isWinPC(body) || OS.isMac(body) || OS.isLinux(body)) && (w.screen.width === 1024 || w.screen.width === 768)){
		  ...
		  case "1.600": // Resolution - 1680×1050, Resolution  - 1440x900, Resolution - 1280×800
		     if((OS.isWinPC(body) || OS.isMac(body) || OS.isLinux(body))){
		        body.className += " 1680×1050";
		        body.className += " 1440×900";
			    body.className += " 1280×800";
			    body.setAttribute("aria-view-mode","desktop"); 
			 }	
		  break;
    ...
    if(!d[dd] && !isPresto){  // a necessary step to avoid conflict with IE/Opera and others...
        ...
        if(OS.isWinPC()){
		       ...
		       // detect for 'Andriod' as well... do this later!!
